...
const WorkoutPlaceDetail = () => {
   const route = useRoute<MyScreenRouteProp>();

   const { finished, bookingId } = route.params;
   ...
   useEffect(() => {
      if (!Object.keys(booking?.data || {}).length && !bookingLoading) {
         toast.show({
            type: 'error',
   ...
   const spaceDetail = booking?.data.spaces;
...
export default WorkoutPlaceDetail;
